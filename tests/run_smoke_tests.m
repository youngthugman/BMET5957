...
config.ecg.qrsMode="supplied"; ecg=zeros(200,1);
[ecgFeatures,ecgNames]=extract_ecg_features_adapter(ecg,200,1,config,100);
assert(isequal(size(ecgFeatures),[1 10])&&numel(ecgNames)==10&&all(isnan(ecgFeatures(1,7:8))), ...
    'ECG extraction failed to represent an empty RR window.');
